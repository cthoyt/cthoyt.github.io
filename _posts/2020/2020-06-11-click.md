...
post]({% post_url 2020/2020-06-03-how-to-code-with-me-organization %}) and
you're now a packaging master, is to import `enlightened_analysis` and run it
from the Python REPL. Another way would be to make a one-off Python script whose
job is to actually run the analysis (as opposed to this example, which is
creating the workflow to be run). Though these are both better than the sinful
analysis, it's a problem since you have to manually interact with Python to run
your code.
...
CLIs should be very, very short! If you're putting lots of logic inside your
CLI, then you should probably reconsider refactoring that logic into more
generally reusable functions. Here's an example of the hello world CLI:
...
If you've done it right, the body of the `main()` function for your CLI should
be a very boring function. Of course, there are other ways to organize your
code, but this is a good way to do it until you're more comfortable.
...
post]({% post_url 2020/2020-06-03-how-to-code-with-me-organization %}) or
...
@click.group()
def main():
    pass  # because this is a group, you don't actually need to do anything in it
