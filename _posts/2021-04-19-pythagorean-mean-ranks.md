...
[Pythagorean means](https://en.wikipedia.org/wiki/Pythagorean_means). This blog
post describes ideas [Max Berrendorf](https://github.com/mberr) shared with me
that I recently implemented in [PyKEEN](https://github.com/pykeen/) and later
wrote up as a [full manuscript](https://arxiv.org/abs/2203.07544).
...
but more generally called _positive unlabeled learning_ in the wider machine
...
This also means that alternative metrics need to be introduced that don't rely
...
[GSEA](https://www.gsea-msigdb.org/gsea) results on differential gene expression
...
In the following definitions, I'm going to use $\\mathcal{I}$ to denote the set
of all ranks for true triples.

### Hits @ K

While this post isn't about hits@k, it's worth summarizing the alternative
...
For example, if Google shows 20 results on the first page, then the percentage
of results that are relevant is the hits @ 20. The hits@k, regardless of $k$,
lies between $(0, 1]$ where closer to 1 is better.

This metric does not differentiate between cases when the rank is larger than
...
The mean rank (MR) computes the arithmetic mean over all individual ranks. It is
given as:
...
While it remains interpretable, the mean rank is dependent on the number of
...
The mean reciprocal rank (MRR) is the arithmetic mean of reciprocal ranks, which
...
[Fuhr (2018)](https://pykeen.readthedocs.io/en/latest/references.html#fuhr2018).
However, this opinion is not undisputed, cf.
[Sakai (2021)](https://pykeen.readthedocs.io/en/latest/references.html#sakai2021).
Despite its _potential_ flaws, MRR is still often used during early stopping due
...
It's odd that the mean rank and mean reciprocal rank are formulated in a way
...
would make sense to report the harmonic mean rank (HMR) as well, defined by:
...
It has the benefit that it's more easily comparable to the mean rank because
...
really the arithmetic mean rank) by:
...
circle of diameter $a+b$.

### Getting Geometrified

For sets of positive numbers (which ranks always are), there is a nice property
...
the geometric mean rank (GMR) as:
...
Like the MR and HMR, $\\text{GMR} \\in [1,\\infty)$. Like with the other
metrics, its inverse, the inverse geometric mean rank (IGMR) can be defined as:
...
While we proposed the geometric mean rank and its inverse after examining the
...
PyKEEN has always been focused on easy usage. If you want to get these metrics,
do the following:
...
The rules for `get_metric()` are outlined
[here](https://pykeen.readthedocs.io/en/latest/api/pykeen.evaluation.RankBasedMetricResults.html#pykeen.evaluation.RankBasedMetricResults.get_metric).
Usually, you should write out the metric name in full, with underscores in the
middle. The common ones like `mrr`, `mr`, and `igmr` can be accessed by synonym.

---

We've implemented all of these statistics in PyKEEN
[pull request #381](https://github.com/pykeen/pykeen/pull/381), reported for the
