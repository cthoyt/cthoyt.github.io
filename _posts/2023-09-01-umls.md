...
Building on top of the automated download of UMLS, I used implemented a fit-for-purpose processor that converts it into
an ontology (encoded either as OWL, OBO, or OBO Graph JSON) which can therefore be used to generate mappings in the 
SSSOM format. The code that implements this can be found [here](https://github.com/pyobo/pyobo/commit/be67eadf77afa270cf224cad71d5b1905ae78b53).

Ultimately, the mappings can be accessed with the following lines of Python code (after `pip install pyobo`):
