...
This has been implemented in the open source [`umls_downloader`](https://github.com/cthoyt/umls_downloader) Python
...
found [here](https://github.com/pyobo/pyobo/commit/be67eadf77afa270cf224cad71d5b1905ae78b53). After installing PyOBO
with `pip install pyobo`, you can automatically download and convert UMLS
first into an ontology encoded in
the [OBO flat file format](https://owlcollab.github.io/oboformat/doc/GO.format.obo-1_4.html),
then convert to OWL with the following code. Note: you'll need [`robot`](https://robot.obolibrary.org/) for the second
step:
...
In an ideal world, the results of such a conversion could be included as a part of the
[OBO Database Ingestion](https://github.com/biopragmatics/obo-db-ingest), which converts database
resources available through PyOBO into ontology artifacts, archives them on GitHub and Zenodo, and gives them PURLs
all on a weekly basis to make sure the most up-to-date version is available as well as all previous named versions.
...
[HPO](https://bioregistry.io/hpo), [LOINC](https://bioregistry.io/loinc), and more. However, there are a few caveats to
consider:

1. UMLS mappings aren't all 1-to-1. For example, MeSH mappings typically include many UMLS terms (narrower) pointing to
   the same MeSH term (broader). For other vocabularies, such as NCBITaxon, UMLS mappings are more reliably 1-to-1.
   Thanks to Tiago Lubiana for pointing this out.
2. Mapping provenance is not available, so the `mapping_justification` field in SSSOM is uniformly filled with
   `sempav:UnspecifiedMatching`.
3. Similarly, UMLS does not apply precise semantic predicates for each mapping. This means that they are output
   in PyOBO and as SSSOM with `oboInOwl:hasDbXref` instead of more detailed types such
   as `skos:exactMatch`, `skos:narrowMatch`, and `skos:broaderMatch`. Tools
   like [Boomer](https://github.com/INCATools/boomer) can be used to address this (in part). The [Semantic Mapping
   Reasoning Assembler (SeMRA)](https://github.com/biopragmatics/semra) can also be configured with prior knowledge
   about UMLS mapping assumptions when aggregating and reasoning over semantic mappings at scale.

With that in mind, anything that can be loaded as an ontology in PyOBO can also be exported with SSSOM, which I show
below. For UMLS, this looks like:
